function [ fval,Xfood,gbest_t] = TE_SO(varargin)
% function [curve, bestx, bestf] = SO(fobj, varargin)
% global optimizer;
% function [Xfood, fval,gbest_t] = SO(N,T,fobj, dim,lb,ub)
%fhd,pos,varargin,fobj
%注释fobj=fhd,X=pos,bestf=fval,xfood=bestx,gbest_t=curve
%注释T=maxgen,N=popsize
%initial 
N = cell2mat(varargin(1));   % 种群大小
T = cell2mat(varargin(2));   % 最大迭代次数
lb = cell2mat(varargin(3));   % 变量下界,low
ub = cell2mat(varargin(4));   % 变量上界,upper
dim = cell2mat(varargin(5));  % 变量个数,dimension
fhd = cell2mat(varargin(6));  % 适应度函数,fhd
...
pos=tent_chaos_init(N,dim,ub,lb);
X=pos;
% freq=1/dim;%频率
for i=1:N
...
%                         %随机变异，替代
%                         if rand<0.5
%                           Xnew1(z1,j)=Xfood(j)+sign(rand-0.50)*(lb+rand*(ub(j)-lb(j)));
%                         else
%                           Xnew1(z1,j)=X(z1,j)+sign(rand-0.50)*(lb+rand*(ub(j)-lb(j)));
%                         end
%                     end
...
% 生成反向种群
 ...
 %sort默认升序，sort(A,'descend')降序，对各行元素升序排列
 % sort(A,2)对各列元素升序
 % I1代表排序的index矩阵
 % ~代表忽略第一个参数，只接收第二个
   [~,I1]=sort(ym);
   ...
   for j=1:Nm
      for i=1:dim
         obl_Xnewm(j,i)=rand*(max(m_elite(:,i))+min(m_elite(:,i)))-Xnewm(j,i);
            if  obl_Xnewm(j,i)>ub(i)||obl_Xnewm(j,i)<lb(i)
   ...
   for i=1:a
    ...
    for j=1:Nf
         for i=1:dim
            ...
            if  obl_Xnewf(j,i)>ub(i)||obl_Xnewf(j,i)<lb(i)
